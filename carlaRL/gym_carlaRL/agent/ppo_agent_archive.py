...
class Actor(nn.Module):
    ...
    def forward(self, image, vehicle_states, actions=None):
        ...
        vehicle_states = torch.as_tensor(vehicle_states, dtype=torch.float32, device=DEVICE)
        if vehicle_states.dim() == 1:
            vehicle_states = vehicle_states.unsqueeze(0)  # Ensure it has batch dimension
        x = torch.cat((x, vehicle_states), 1)
        # print(f'input shape to fc_layers: {x.shape}')
        x = self.fc_layers(x)

        mu = torch.tanh(x) * 0.2  # Scale to action space
        self.mu = mu
...
class Critic(nn.Module):
    ...
    def forward(self, image, vehicle_states):
        x = torch.as_tensor(image).float().to(DEVICE)
        if x.dim() == 3:
            x = x.unsqueeze(0)
        x = self.conv_layers(x)
        vehicle_states = torch.as_tensor(vehicle_states, dtype=torch.float32, device=DEVICE)
        if vehicle_states.dim() == 1:
            vehicle_states = vehicle_states.unsqueeze(0)
        x = torch.cat((x, vehicle_states), 1)
        x = self.fc_layers(x)
...
class ActorCritic(nn.Module):
    ...
    def forward(self, image, vehicle_states):
        action, logp = self.pi(image, vehicle_states)
        value = self.v(image, vehicle_states)
    ...
    def compute_pi_loss(self, images, vehicle_states, actions, steer_guides, advantages, logps_old, clip_ratio=0.2, beta=0.01):
        ...
        logps, means, entropy = self.pi(images, vehicle_states, actions)
        # print(f'means: \n {means.squeeze(-1)} \n guide steers: \n {steer_guides}')
        ...
        loss_pi = loss_ppo + loss_imitation * 5
    ...
    def compute_v_loss(self, images, vehicle_states, returns):
        value = self.v(images, vehicle_states)
